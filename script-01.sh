...
#	REALM: 	TI-PRO.INTRA
#	SERVER:	dc01ls.ti-pro.intra
#	ADMIN:	dc01ls.ti-pro.intra
		...
		then
			if [ "$KERNEL" == "4.4" ]
				then
					 ...
					 echo
					 echo -e "1 - NTP (Network Time Protocol) Servidor de Data é Hora"
					 echo -e "2 - KRB5 (Kerberos) Protocolo de Autenticação Segura"
					 echo -e "3 - NFS (Network File System) Protocolo de Transferência de Arquivos"
					 echo -e "4 - ACL (Access Control List) Permissões de Arquivos e Diretórios"
					 echo -e "5 - ATTR (Extended Attributes) Atributos Extendidos"
					 echo -e "6 - Configuração do FSTAB para suporte a ACL e XATTR"
					 echo
					 echo -e "Após o término o Servidor $SERVERS será reinicializado, aguarde..."
					 echo
					 ...
					 echo -e "Pressione <Enter> para concluir o processo e reinicializar o servidor $SERVERS."
					 read
					 sleep 2
					 reboot
					 else
