...
#	vim /etc/hosts
#	192.168.1.10	dc01ls.ti-pro.intra	dc01ls
#
		...
		then
			if [ "$KERNEL" == "4.4" ]
				then		
					 ...
					 echo -e "1 Usuário é `whoami`, continuando a executar o Script-02.sh"
					 echo
					 echo -e "2 Instalação dos software: SAMBA-4, DNS, CUPS, DHCP, WINBIND e QUOTA"
					 echo
					 echo -e "3 SAMBA-4 (Server Message Block) Serviço de Armazenamento e Gerenciamento de Arquivos e Usuários"
					 echo -e "4 DNS (Domain Name System) Serviço de Domínio de Nomes"
					 echo -e "5 CUPS (Common Unix Printing System) Serviços de Impressão"
					 echo -e "6 Para testar o CUPS após a instalação acesse a URL: http://`hostname -I`:631"
					 echo -e "7 DHCP (Dynamic Host Configuration Protocol) Configuração Dinâmica de Computadores"
					 echo -e "8 PAM (Pluggable Authentication Modules for Linux) Autenticação integrada"
					 echo -e "9 WINBIND Integração SAMBA-4 + Linux"
					 echo -e "10 QUOTA Criação de Quotas de Discos"
					 echo -e "11 CLAMAV Sistema de Anti-Vírus Open Source"
					 echo
					 echo -e "12 Configuração do FSTAB para suporte a Quota"
					 echo -e "13 Configuração do NSSWITCH para suporte a Winbind"
					 echo -e "14 Configuração do CLAMAV para suporte a Anti-vírus"
					 echo -e "15 Configuração do HOSTNAME para suporte a FQDN"
					 echo -e "16 Configuração do HOSTS para suporte a DNS local"
					 echo -e "17 Configuração do GRUB para ativar o recurso de Alias de Rede"
					 echo -e "18 Configuração do CUPS para suporte a configuração remota"
					 echo
					 echo -e "Aguarde..."
					 echo 
					 ...
					 apt-get -y install samba samba-common smbclient cifs-utils samba-vfs-modules samba-testsuite samba-dbg samba-dsdb-modules &>> $LOG
				   echo -e "Instalação do samba-4 concluido..."
				   apt-get -y install cups cups-bsd cups-common cups-core-drivers cups-pdf printer-driver-gutenprint printer-driver-hpcups &>> $LOG
					 echo -e "Instalação do cups concluido..."
					 apt-get -y install hplip openprinting-ppds cups-pk-helper &>> $LOG
				   echo -e "Instalação do hplip concluido..."
				   apt-get -y install antiword docx2txt gutenprint-doc gutenprint-locales isc-dhcp-server &>> $LOG
			 	   echo -e "Instalação do antword, docx2txt concluido..."
			 	   apt-get -y install winbind quota quotatool ldb-tools libnss-winbind libpam-winbind &>> $LOG
			 		 echo -e "Instalação do winbind concluido..."
			 		 apt-get -y install nmap bind9 bind9utils &>> $LOG
					 echo -e "Instalação do nmap concluido..."
					 apt-get -y install clamav clamav-base clamav-daemon clamav-freshclam clamdscan clamfs clamav-testfiles clamav-unofficial-sigs &>> $LOG
					 echo -e "Instalação do clamav concluido..."
					 apt-get -y install arc cabextract p7zip unzip unrar libclamunrar7 kcc tree &>> $LOG
					 echo -e "Instalação do arc, cabextract, p7zip concluido..."
					 ...
					 echo
					 echo -e "Informação a ser acrescentadas na linha 03: 192.168.1.10 dc01ls.ti-pro.intra dc01ls"
					 echo
