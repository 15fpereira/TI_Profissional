		...
		then
			if [ "$KERNEL" == "4.4" ]
			 ...
			 clear
					 ...
					 setfacl -R -m user:"TI-PRO\Guest":rwx -d $PATHFILE/$FILEACL
					 setfacl -R -m group:"TI-PRO\Domain Admins":rwx -d $PATHFILE/$FILEACL
					 setfacl -R -m group:"TI-PRO\Domain Guests":rwx -d $PATHFILE/$FILEACL
					 echo
					 ...
					 echo -e "04. Criando as ACL para o arquivo: $FILEATTR em: $PATHFILE"
					 echo
					 ...
					 setfattr -n user.autor -v "Francisco Dias" $PATHFILE/$FILEATTR
					 setfattr -n user.depto -v "Organizacao" $PATHFILE/$FILEATTR
